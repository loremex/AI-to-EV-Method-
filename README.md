...
File: AI_to_EV_Workbook_lv_v43.xlsx Case: Contoso (vertical SaaS) Tabs: 11 · Formulas: 1,145 · Errors: 0 (verified by recalc)
...
Score 36 capabilities against a frontier rubric.
Distance from the frontier sets how much of each value pool is reachable.
Reachable pool is cut by feasibility, then by timing.
What survives splits into a revenue half and a cost half.
The revenue half lifts ARR. The whole pool lifts growth. Growth lifts the multiple.
New ARR × new multiple = new enterprise value. Subtract the NPV of program cost.
